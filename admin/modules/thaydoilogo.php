...
if(isset($_POST['check'])){
    if($_FILES['imgInp']['error'] == 0)
    {
        ...
        if (in_array($imageFileType,$allowtypes ))
        {
            ...
            $sql = "UPDATE tuychinhweb SET giatri='$pathav' WHERE thuoctinh = 'logo'";
    ...
    echo '<script>window.location="'.$domain.'/admin/?chucnang=thaydoilogo";</script>';
}
?>
...
<div style="background: #fff; padding: 20px; margin: auto; width: 350px; margin-top: 60px; border-radius: 3px; box-shadow: 0px 0px 3px 3px #95999c; ">
    <div style="text-align: center;">  <H4>Thay đổi Logo Web</H4></div>
    <form method="post" action="#"  enctype="multipart/form-data">
        <input style="display: none" name="check">
        <div class="form-group">
            <?php
            $getFavicon = mysqli_fetch_assoc(mysqli_query($conn, "SELECT giatri FROM tuychinhweb where thuoctinh = 'logo'"))['giatri'];
            ?>
